...
def main():
	...
	pygame.display.set_caption('来自一位喜欢你的小哥哥-Benjamin')
	# 背景音乐
		...
		for event in pygame.event.get():
			if event.type == pygame.QUIT:
				...
				if button_yes.rect.collidepoint(pygame.mouse.get_pos()):
					...
					root = Tk()
					root.withdraw()
					messagebox.showinfo('', '很高兴遇见你')
					root.destroy()
					is_agree = True
		# --显示文字
		showText(screen=screen, text='我喜欢你很久了', position=(40, 30), 
				 fontpath=cfg.FONT_PATH, fontsize=25, fontcolor=cfg.BLACK, is_bold=False)
		showText(screen=screen, text='做我女朋友好不好?', position=(40, 80), 
				 fontpath=cfg.FONT_PATH, fontsize=25, fontcolor=cfg.BLACK, is_bold=False)
		showText(screen=screen, text='-----赵兵（benjamin）', position=(40, 130), 
				 fontpath=cfg.FONT_PATH, fontsize=25, fontcolor=cfg.BLACK, is_bold=False)		 
		# --显示按钮
